...
const EditTicketScreen = ({ route, navigation }) => {
  ...
  const extractLabel = (val) => {
    ...
    return val.companyName || val.name || val.label || val.title || val.subject || val.categoryId || 'Unknown';
  };
  
  const extractValue = (val) => {
    if (!val) return null;
    if (typeof val === 'string') return val;
    return val._id || val.value || val.id || extractLabel(val);
  };

  const [client, setClient] = useState(ticket.client ? { label: extractLabel(ticket.client), value: extractValue(ticket.client) } : null);
  const [building, setBuilding] = useState(ticket.building ? { label: extractLabel(ticket.building), value: extractValue(ticket.building) } : null);
  const [assignedTo, setAssignedTo] = useState(ticket.assignedTo ? { label: extractLabel(ticket.assignedTo), value: extractValue(ticket.assignedTo) } : null);
  
  const catVal = ticket.category ? (typeof ticket.category === 'string' ? ticket.category : (ticket.category.categoryId || ticket.category.name || 'General')) : null;
  const subcatVal = ticket.subcategory ? (typeof ticket.subcategory === 'string' ? ticket.subcategory : ticket.subcategory.label) : (typeof ticket.category === 'object' ? ticket.category.subCategory : null);
  
  const [category, setCategory] = useState(catVal ? { label: catVal, value: catVal } : null);
  const [subcategory, setSubcategory] = useState(subcatVal ? { label: subcatVal, value: subcatVal } : null);
